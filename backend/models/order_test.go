...
/*
This code is a test file for the OrderDaoPG struct, which provides methods to interact with the database for managing orders. Let's go through the code step by step:

The Test_PG_GetAccountOrders function is a unit test that verifies the functionality of retrieving account orders from the database. It sets up the necessary environment and initializes the test database. Then it performs several assertions to check if the orders are correctly retrieved based on different parameters such as account, market, and order status.

The Test_PG_GetMarketPendingOrders function is another unit test that tests the retrieval of pending orders for a specific market. It initializes the test database, inserts a few orders for the "WETH-DAI" market, and asserts that the correct number of pending orders is returned.

The Test_PG_FindNotExistOrder function tests the scenario where an order with a non-existent ID is searched in the database. It asserts that the returned order is nil, indicating that the order doesn't exist.

The Test_PG_InsertAndFindOneAndUpdateOrders function tests the insertion, retrieval, and update of orders in the database. It inserts a randomly generated order, retrieves it from the database, updates its available and pending amounts, and then verifies if the update was successful by retrieving the order again and comparing the updated values.

The Test_PG_Order_GetOrderJson function tests the GetOrderJson method of the Order struct. It creates a sample OrderJSON object, converts it to JSON format, assigns the JSON string to an order's JSON field, and then asserts that the GetOrderJson method returns the same values as the original OrderJSON object.

The NewOrder function is a helper function that generates a new Order object with random values for testing purposes.

The RandomMatchOrder function generates a pair of matching orders (a maker order and a taker order) with opposite sides (buy and sell).

The RandomOrder function generates a random Order object with random values for testing purposes.
*/